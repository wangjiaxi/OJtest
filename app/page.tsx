...
int main(int argc, char* argv[]) {
  ...
  const [standardCode, setStandardCode] = useState(`#include <iostream>
using namespace std;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-xl font-semibold mb-4">数据生成器</h2>
              <textarea
                value={generatorCode}
                onChange={(e) => setGeneratorCode(e.target.value)}
                className="w-full h-64 font-mono text-sm border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            {/* 第二行：标准程序 */}
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-xl font-semibold mb-4">标准程序</h2>
              <textarea
                value={standardCode}
                onChange={(e) => setStandardCode(e.target.value)}
                className="w-full h-64 font-mono text-sm border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="输入标准程序代码..."
              />
              
              <div className="mt-4 flex items-center gap-4">
                <button
                  onClick={() => localStorage.setItem('standardCode', standardCode)}
                  className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700"
                >
                  保存代码
                </button>
        ...
        <div className="mt-8 bg-white rounded-lg shadow-md p-6">
          ...
          <div className="prose text-gray-700">
            <ol className="list-decimal list-inside space-y-2">
              <li>在左侧第一行编辑器中输入你的testlib生成器代码</li>
              <li>在左侧第二行编辑器中输入标准程序代码</li>
              <li>设置要生成的测试点数量（1-20个）</li>
              <li>点击"生成测试数据"按钮</li>
              <li>查看右侧生成的测试数据，可以下载保存</li>
            </ol>
